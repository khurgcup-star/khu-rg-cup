...
꿈나무부·선수부 화면에서 선수 개인 연락처를 없앤 뒤 이 오류가 나오면,
홈페이지는 최신이지만 Google Apps Script 웹 앱이 이전 버전으로 배포된 상태입니다.

- 이 수정본의 `script.js`는 구형 배포본과도 호환되도록 선수 번호 대신 지도자 연락처를 `phone` 호환값으로 보냅니다.
- 최신 `Code.gs`는 꿈나무부·선수부의 `phone` 값을 저장 전에 비우므로 선수 개인 연락처로 저장되지 않습니다.
- Apps Script에서 `배포 관리 > 수정 > 버전: 새 버전 > 배포`까지 완료하면 정상 구조로 동작합니다.
- 웹 앱 주소를 브라우저에서 열었을 때 `version`이 `2026-07-31-youth-coach-contact-v2`인지 확인하면 됩니다.
...
```text
2026-07-31-youth-coach-contact-v2
```
